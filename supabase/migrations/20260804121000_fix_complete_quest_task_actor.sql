-- Исправляет конфликт имени переменной с системным current_user PostgreSQL.
...
begin
  ...
  where id = target_plan and user_id = actor_user_id
  for update;
  ...
  values
    (actor_user_id, plan_row.goal, target_step, coalesce(task_notes, ''), task_chat, 'done', now(), now())
  ...
  if newly_completed then
    ...
    where cp.challenge_id = c.id and c.status = 'active'
      ...
      and cp.user_id = actor_user_id and cp.invitation_status = 'accepted';
  end if;
