...
"resources":[]}]}.
Верни ровно ${remaining.length} этапов. Не придумывай факты, которых нет в записях.`,
...
Находи подтверждённые пробелы и успехи, постепенно меняй сложность. Отвечай только валидным JSON на русском.`,
  ...
  try {
    ...
    if (!Array.isArray(parsed.steps) || parsed.steps.length !== remaining.length) throw new Error('Incomplete route');
    const future = parsed.steps.map((step, index) => buildStep(remaining[index], step, index === 0));
