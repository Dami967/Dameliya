...
export function ChallengeModal({ friends, onClose, onCreate }: { friends: SocialUser[]; onClose: () => void; onCreate: (challenge: ChallengeDraft) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="social-modal creation-modal challenge-form" onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => {
    ...
    onCreate({ title: form.get('title')?.toString() || challengeLabels[form.get('type') as ChallengeDraft['type']], participantIds: selected, type: form.get('type') as ChallengeDraft['type'], startsAt: form.get('startsAt')?.toString() ?? '', endsAt: form.get('endsAt')?.toString() ?? '', reward: form.get('reward')?.toString().trim() ?? '' });
  }}>
